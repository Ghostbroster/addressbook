    ...
    public static TaskService createDemoService() {
        if (instance == null) {
            ...
            for (int i = 0; i < 100; i++) {
                ...
                task.setEmail(task.getFirstName().toLowerCase() + "@" + task.getLastName().toLowerCase() + ".com");
                task.setPhone("+ 358 555 " + (100 + r.nextInt(900)));
                task.setTaskDescription("Fight " + fnames[r.nextInt(fnames.length)] + ".");
                cal.set(1930 + r.nextInt(70), r.nextInt(11), r.nextInt(28));
                task.setStartDate(cal.getTime());
                task.setExpectedEndDate(new Date());
